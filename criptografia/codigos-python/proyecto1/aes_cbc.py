...
def ayuda():
    ...
    Script que hace implementación manual de AES CBC:
        ...
        - Usar el padding visto clase
        - Generar un IV aleatorio al cifrar, este se pega al final del archivo
    ...
    with open(archivo_salida, "wb") as salida:
        with open(archivo_entrada, "rb") as entrada:
            ...
            while len(chunk) == CHUNK_SIZE:
                ...
                # XOR
                chunk_xor = xor(chunk, bloque_anterior)

                cifrado = aesEncryptor.update(chunk_xor)
            ...
            chunk_xor = xor(chunk + padding, bloque_anterior)
            cifrado = aesEncryptor.update(chunk_xor)
...
def descifrar(archivo_entrada: str, archivo_salida: str, llave: bytes) -> None:
    ...
    with open(archivo_entrada, "rb") as entrada:
        #Obtener IV al final del archivo (16 bytes)
        entrada.seek(-16, 2)
        iv = entrada.read(16)

        #Obtener hash al final del archivo (32 bytes)
        entrada.seek(-48, 2)
        hash_guardado = entrada.read(32)

        #Obtener tamaño de los bloques cifrados
        ...
        with open(archivo_salida, "wb") as salida:
            ...
            while contador < tamano:
                ...
                # XOR con iv/chunk_anterior para CBC
                descifrado = xor(descifrado, bloque_anterior)
                bloque_anterior = chunk_cifrado
            ...
            # Último bloque: quitar padding y escribir
            if chunk_anterior is not None:
