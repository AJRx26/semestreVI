...
def generar_mac(nonce, mensaje, llave_mac):
    h = hmac.HMAC(llave_mac, hashes.SHA256(), backend=default_backend())
    h.update(nonce + mensaje)
    return h.finalize()
...
def leer_mensaje(socket, llave_cifrado, llave_mac):
    ...
    mensaje_descifrado = descifrar_mensaje(nonce=mensaje[:16], mensaje_cifrado=mensaje[16:-len(DELIMITADOR)-32], llave_cifrado=llave_cifrado)
