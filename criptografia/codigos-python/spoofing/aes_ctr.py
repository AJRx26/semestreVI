...
def cifrar(archivo_entrada: str, archivo_salida: str) -> None:
    ...
    print("Nonce del archivo cifrado: ", base64.b64encode(nonce).decode())

def descifrar(archivo_entrada: str, archivo_salida: str, key: bytes, nonce: bytes) -> None:
    """
    ...
    with open(archivo_entrada, "rb") as entrada:
        ...
        size = os.path.getsize(archivo_entrada)
        entrada.seek(0)
...
if __name__ == "__main__":
    ...
    all_args.add_argument("-n", "--nonce", help="Nonce en base64", required=False)
    ...
    if operacion == "cifrar":
        ...
        if len(nonce) != SIZE:
            print("El nonce de entrada debe ser de 16 bytes")
            exit(1)
        descifrar(args["input"], args["output"], llave, nonce)
