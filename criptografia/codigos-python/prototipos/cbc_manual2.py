        ...
        with open(archivo_entrada, "rb") as entrada:
            ...
            while len(chunk) == CHUNK_SIZE:
                ...
                for i in range(0, len(chunk), BLOCK_SIZE):
                    ...
                    # ECB
                    cifrado_bloque = encryptor.update(bloque_xor)
                    #Gurada el bloque cifrado para el siguiente ciclo
                    bloque_anterior = cifrado_bloque
            ...
            for i in range(0, len(chunk_con_padding), BLOCK_SIZE):
                #toma un chunk desde i (0) hasta (15) dado que el tamaño del bloque es 16
                bloque = chunk_con_padding[i:i + BLOCK_SIZE]
                # XOR
                bloque_xor = xor(bloque, bloque_anterior)
                # ECB
                cifrado_bloque = aesEncryptor.update(bloque_xor)
                #Gurada el bloque cifrado para el siguiente ciclo
                bloque_anterior = cifrado_bloque
                cifrado += cifrado_bloque
...
def descifrar(archivo_entrada: str, archivo_salida: str, llave: bytes) -> None:
        ...
        with open(archivo_salida, "wb") as salida:
            ...
            while contador < tamano:
                ...
                for i in range(0, len(chunk), BLOCK_SIZE):
                    ...
                    bloque = chunk[i:i + BLOCK_SIZE]
                    #Descifrado ECB
                    descifrado_bloque = aesDecryptor.update(bloque)
                    # XOR
                    descifrado_xor = xor(descifrado_bloque, bloque_anterior)
                    #Gurada el bloque cifrado para el siguiente ciclo
                    bloque_anterior = bloque
                    descifrado += descifrado_xor
    ...
    if hash_calculado == hash_guardado:
        print("Integridad verificada: archivo NO modificado")
    else:
        print("ADVERTENCIA: el archivo fue modificado")
